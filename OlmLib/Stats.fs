    ...
    let Get (igms:(Hdr*FsChess.Types.Move)[],bd:FsChess.Types.Brd,cp:ChessPack) =
        let rec createstats (cigmmvl:(Hdr*FsChess.Types.Move) list) (bdst:BrdStats) =
            if cigmmvl.IsEmpty then bdst
                ...
                let result = hdr.Result|>GameResult.Parse
                ...
                if cmvstl.IsEmpty then 
                    let mvst = {Mv=mv;Mvstr=mv|>FsChess.Move.ToSan bd;Count=1;Pc=0.0;
                                WhiteWins=(if result=GameResult.WhiteWins then 1 else 0);
                                Draws=(if result=GameResult.Draw then 1 else 0);
                                BlackWins=(if result=GameResult.BlackWins then 1 else 0);
                                Score=0.0;DrawPc=0.0}
                    createstats cigmmvl.Tail {bdst with TotCount=(bdst.TotCount+1);
                                                        TotWhiteWins=(if result=GameResult.WhiteWins then bdst.TotWhiteWins+1 else bdst.TotWhiteWins);
                                                        TotDraws=(if result=GameResult.Draw then bdst.TotDraws+1 else bdst.TotDraws);
                                                        TotBlackWins=(if result=GameResult.BlackWins then bdst.TotBlackWins+1 else bdst.TotBlackWins);
                                                        Mvstats=mvst::bdst.Mvstats}
                else
                    let cmvst = cmvstl.Head
                    let mvst = {cmvst with Count=(cmvst.Count+1);
                                           WhiteWins=(if result=GameResult.WhiteWins then cmvst.WhiteWins+1 else cmvst.WhiteWins);
                                           Draws=(if result=GameResult.Draw then cmvst.Draws+1 else cmvst.Draws);
                                           BlackWins=(if result=GameResult.BlackWins then cmvst.BlackWins+1 else cmvst.BlackWins)}
