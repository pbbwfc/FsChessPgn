...
module MoveUtil = 
        ...
        if fits.Length <> 1 then 
            let rec getfits (mvl : Move list) fitl = 
                if List.isEmpty mvl then fitl
                    ...
                    elif file <> FILE_EMPTY && ((mv|>Board.From)|>Square.ToFile) <> file then getfits mvl.Tail fitl
                    elif rank <> RANK_EMPTY && (mv|>Board.From|>Square.ToRank) <> rank then getfits mvl.Tail fitl
    ...
    let Desc(move : Move) = 
        (move|>Board.From|>Square.Name).ToLower() + (move|>Board.To|>Square.Name).ToLower() 
        + (if move|>Board.Promote <> Piece.EMPTY then (move|>Board.Promote|>Piece.PieceToString).ToLower() else "")
    
    let DescBd (move : Move) (board : Brd) = 
        let sb = new StringBuilder()
        let piece = board.PieceAt.[int(move|>Board.From)]
        let fromrank = move|>Board.From|>Square.ToRank
        let fromfile = move|>Board.From|>Square.ToFile
        let isprom = move|>Board.Promote <> Piece.EMPTY
        let sTo = move|>Board.To|>Square.Name
        ...
        elif piece = Piece.WKing && (move|>Board.From) = E1 && (move|>Board.To) = G1 then 
            sb.Append("O-O") |> ignore
        elif piece = Piece.BKing && (move|>Board.From) = E8 && (move|>Board.To) = G8 then 
            sb.Append("O-O") |> ignore
        elif piece = Piece.WKing && (move|>Board.From) = E1 && (move|>Board.To) = C1 then 
            sb.Append("O-O-O") |> ignore
        elif piece = Piece.BKing && (move|>Board.From) = E8 && (move|>Board.To) = C8 then 
            sb.Append("O-O-O") |> ignore
        else 
